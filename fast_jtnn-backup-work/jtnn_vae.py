...
class JTNNVAE(nn.Module):
    ...
    def __init__(self, vocab, hidden_size, latent_size, depthT, depthG):
        ...
        self.latent_size = int(latent_size // 2)  # Tree and Mol have two vectors

        self.jtnn = JTNNEncoder(self.hidden_size, int(depthT), nn.Embedding(vocab.size(), self.hidden_size))
        self.decoder = JTNNDecoder(vocab, self.hidden_size, self.latent_size, nn.Embedding(vocab.size(), self.hidden_size))

        self.jtmpn = JTMPN(self.hidden_size, int(depthG))
        self.mpn = MPN(self.hidden_size, int(depthG))

        self.A_assm = nn.Linear(self.latent_size, self.hidden_size, bias=False)
